...
if($antigo != $cpf){
    $query = $pdo->query("SELECT * FROM oficina.mecanico WHERE cpf = '$cpf'");
...
if($antigo2 != $email){
    $query = $pdo->query("SELECT * FROM oficina.mecanico WHERE email = '$email'");
    $res = $query->fetchAll(PDO::FETCH_ASSOC);
    $total_reg = @count($res);
    if($total_reg > 0){
...
if($id == ""){
    $res = $pdo->prepare("INSERT INTO oficina.mecanico SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone");   
    ...
    $res = $pdo->prepare("UPDATE oficina.mecanico SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone WHERE id = '$id'");
